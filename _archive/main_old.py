...
def convert_file(file_path: Path, source_root: Path, dest_root: Path, status_callback,
    ...
    if ext == ".pdf":
        process_pdf(file_path, media_dir, file_uuid, md_path,
                    warn_log, conv_log, proc_log, model_dict=model_dict)
    elif ext == ".docx":
        process_docx(file_path, media_dir, file_uuid, md_path,
                    warn_log, conv_log, proc_log)
