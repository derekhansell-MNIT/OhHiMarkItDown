...
def run_marker_single(pdf_path, pdf_converter):
    """
    Convert a PDF to Markdown using a shared Marker PdfConverter.
    """
    try:
        rendered = pdf_converter(str(pdf_path))
...
def process_pdf(pdf_path, media_dir, uuid, md_path, warn_log, info_log, proc_log, pdf_converter):
    """
    Full PDF processing pipeline:
    1. Convert PDF to Markdown using shared Marker PdfConverter (GPU-accelerated)
    ...
    try:
        md_result = run_marker_single(pdf_path, pdf_converter)
