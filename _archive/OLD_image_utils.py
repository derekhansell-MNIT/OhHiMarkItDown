...
def rewrite_markdown_images(md_path: Path, rel_paths: list, info_log: Path, warn_log: Path):
    ...
    - Second pass: Marker-style local filenames (e.g., _page_0_Picture_4.jpeg)
    The order of rel_paths is used sequentially.
    """
    try:
    ...
    # Pass 1: replace data:image
    text = DATA_IMG_RE.sub(repl, text)
    # Pass 2: replace Marker/local filenames
    text = LOCAL_MARKER_IMG_RE.sub(repl, text)
